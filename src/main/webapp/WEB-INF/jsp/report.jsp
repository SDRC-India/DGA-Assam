			...
			<div class="select-container text-center col-md-3">
				<div class="input-group ">
					...
					<div class="input-group-btn" style="position: relative;">
						...
						<ul class="dropdown-menu" role="menu">
							<li ng-repeat="timeperiod in sectorTimeperiods"
					...
					<tbody>
						<tr
							...
							<td ng-repeat="column in columns | filter:removeRowId"
								...
								ng-click="$first && selectedFacility.iC_NId != 17 && !$parent.$last?getBlockTable(rowData):''">{{rowData[column]?filterFloat(rowData[column]): "-"}}</td>
						</tr>
					</tbody>
				</table>
			</div>
							...
							<tbody>
								<tr
									...
									<td ng-repeat="column in divisionalTableColumns"
										sortable="'{{rowData.column}}'">{{rowData[column]? filterFloat(rowData[column]):"-"}}</td>
								</tr>
							</tbody>
						</table>
					</div>
