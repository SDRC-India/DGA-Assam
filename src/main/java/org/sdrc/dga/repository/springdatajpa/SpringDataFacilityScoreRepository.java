...
public interface SpringDataFacilityScoreRepository extends
	...
	@Override
	@Query("SELECT fxm.label,(fc.score / fxm.maxScore)*100,fc.lastVisitData.timPeriod.timeperiod FROM "
	...
	@Override
	@Query("SELECT fxm.label,(AVG(fc.score) / fxm.maxScore)*100,fc.lastVisitData.timPeriod.timeperiod FROM "
			+ "FacilityScore fc JOIN fc.formXpathScoreMapping fxm "
			+ "WHERE  fxm.parentXpathId=:parenXpathId "
			...
			+ " GROUP BY fxm.label ,fxm.maxScore,fc.lastVisitData.timPeriod.timeperiod "
			+ "ORDER BY fxm.label ASC ")
	public List<Object[]> findSpiderDataChartByFormId(
			@Param("formId") Integer formId,@Param("parenXpathId")Integer parenXpathId,@Param("timeperiodId")int timeperiodId);
